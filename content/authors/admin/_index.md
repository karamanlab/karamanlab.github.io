---
title: Ferhat Karaman

first_name: Ferhat
last_name: Karaman

superuser: true

role: Assistant Professor of Psychology

organizations:
  - name: Texas A&M University-Kingsville
    url: "https://www.tamuk.edu"

bio: I study language acquisition, statistical learning, and memory using experimental methods.

interests:
  - Language Acquisition
  - Statistical Learning
  - Memory

education:
  courses:
    - course: PhD in Experimental Psychology
      institution: University of Tennessee, Knoxville
      year: 2018
    - course: MA in Experimental Psychology
      institution: University of Tennessee, Knoxville
      year: 2015
    - course: MA in Experimental Psychology
      institution: Florida Atlantic University
      year: 2013

social:
  - icon: envelope
    icon_pack: fas
    link: "mailto:ferhat.karaman@tamuk.edu"
  - icon: twitter
    icon_pack: fab
    link: "https://twitter.com/KaramanFerhat"
  - icon: google-scholar
    icon_pack: ai
    link: "https://scholar.google.com/citations?user=p9srGFwAAAAJ&hl=en&oi=ao"
  - icon: github
    icon_pack: fab
    link: "https://github.com/ferhatkaraman"

email: ""

highlight_name: true

user_groups:
  - Lab Director
---

I am an Assistant Professor of Psychology at Texas A&M University–Kingsville. My research focuses on language learning and cognition across development, with a particular emphasis on how learners extract patterns and structure from linguistic input.

Using behavioral and experimental methods, I investigate language processing in children and adults and examine how statistical learning mechanisms support language acquisition.
